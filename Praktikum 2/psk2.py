...
class psk2(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "psk2")
        ...
        self.qam16 = qam16 = [-1-1j, 1-1j, 1+1j, -1+1j, 1+3j, 3+1j, 3+3j, -1+3j, -3+1j, -3+3j, 3+3j, -1+3j, -3+3j, 3-1j, 1-3j, 3-3j, -3-1j, -1-3j, -3-3j]
        self.noise = noise = 0
        self.mod_order = mod_order = 4
        self.fco = fco = 0
        self.constellation = constellation = -1, 1
        ...
        self._constellation_options = [(-1, 1), [(-1-1j), (1-1j), (1+1j), (-1+1j)], [(-1-1j), (1-1j), (1+1j), (-1+1j), (1+3j), (3+1j), (3+3j), (-1+3j), (-3+1j), (-3+3j), (3+3j), (-1+3j), (-3+3j), (3-1j), (1-3j), (3-3j), (-3-1j), (-1-3j), (-3-3j)]]
        ...
        self._fco_range = qtgui.Range(0, 100000, 10, 0, 200)
        self._fco_win = qtgui.RangeWidget(self._fco_range, self.set_fco, "'fco'", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_layout.addWidget(self._fco_win)
...
def main(top_block_cls=psk2, options=None):
